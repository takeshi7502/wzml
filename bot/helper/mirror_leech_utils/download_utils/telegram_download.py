...
class TelegramDownloadHelper:
    def __init__(self, listener):
        ...
        self.session = ""
        self._hyper_dl = (
            Config.USE_HYPER and len(TgClient.helper_bots) != 0 and Config.LEECH_DUMP_CHAT
        )
        ...
        try:
            ...
            if self._hyper_dl:
                try:
                    ...
                    LOGGER.warning(f"HyperTG failed, falling back to normal Telegram download: {e}")
    ...
    async def add_download(self, message, path, session):
        ...
        if not self.session:
            if self._hyper_dl:
                self.session = "hbots"
            elif self._listener.transmission_mode == "user" and self._listener.is_super_chat:
                self.session = "user"
                try:
