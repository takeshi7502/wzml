        ...
        try:
            ...
            if dump_chat:
                ...
                if TgClient.user is not None:
                    copy_clients.append(TgClient.user)
                copy_clients.append(TgClient.bot)
                last_error = None
                for copy_client in copy_clients:
                    try:
                        await copy_client.get_chat(dump_chat)
                        copied = await copy_client.copy_message(
                            chat_id=dump_chat,
                            from_chat_id=message.chat.id,
                            message_id=message.id,
                            disable_notification=True,
                        )
                        copied = await copy_client.get_messages(
                            chat_id=dump_chat, message_ids=copied.id
                        )
                        await self.get_media_type(copied)

                        helper_ok = False
                        for helper in self.clients.values():
                            try:
                                helper_msg = await helper.get_messages(
                                    chat_id=dump_chat, message_ids=copied.id
                                )
                                await self.get_media_type(helper_msg)
                                helper_ok = True
                                break
                            except Exception as e:
                                last_error = e
                        if not helper_ok:
                            raise ValueError(
                                "copied media is not visible as media to any helper bot"
                            )
                        ...
                        last_error = e
                        LOGGER.warning(
                            f"Failed to prepare dump media {dump_chat} using "
                            f"{getattr(copy_client.me, 'username', 'user')}: {e}"
                        )
                if self.message is None:
                    raise last_error or ValueError(f"Cannot access dump chat {dump_chat}")
                await sleep(3)
