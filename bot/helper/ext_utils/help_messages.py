...
def _jinmups_help_lines(group):
    from ..telegram_helper.bot_commands import BotCommands
    from ..telegram_helper.ui_themes import blockquote, format_jinmups_command

    descriptions = {
        "Mirror": "Start mirroring to cloud.",
        "QbMirror": "Start Mirroring to cloud using qBittorrent.",
        "JdMirror": "Start Mirroring to cloud using JDownloader.",
        "NzbMirror": "Start Mirroring to cloud using Sabnzbd.",
        "Ytdl": "Mirror yt-dlp supported link.",
        "UpHoster": "Upload to DDL Servers.",
        "Leech": "Start leeching to Telegram.",
        "QbLeech": "Start leeching using qBittorrent.",
        "JdLeech": "Start leeching using JDownloader.",
        "NzbLeech": "Start leeching using Sabnzbd.",
        "YtdlLeech": "Leech yt-dlp supported link.",
        "Clone": "Copy file/folder to Google Drive.",
        "Count": "Count file/folder of Google Drive.",
        "Delete": "Delete file/folder from Google Drive.",
        "UserSet": "Users settings.",
        "BotSet": "Bot settings.",
        "Select": "Select files from torrents or nzb by gid or reply.",
        "CancelTask": "Cancel task by gid or reply.",
        "ForceStart": "Force start task by gid or reply.",
        ...
        "RestartSessions": "Restart user sessions.",
        "Log": "Get a log file of the bot.",
        "Shell": "Run shell commands.",
        "AExec": "Exec async functions.",
        "Exec": "Exec sync functions.",
        "ClearLocals": "Clear aexec or exec locals.",
        "Rss": "RSS Menu.",
        "IMDB": "Get IMDB info.",
        "NzbSearch": "Search NZB indexers.",
        "Plugins": "Manage bot plugins.",
        "Broadcast": "Broadcast message to bot users.",
        "SpeedTest": "Check Bot Speed using Speedtest.com.",
        "SpeedTestModule": "Check Bot Speed using Speedtest.com.",
    }

    commands = BotCommands.get_commands()
    lines = []
    for key, cmds in commands.items():
        is_admin = key in JINMUPS_ADMIN_COMMAND_KEYS
        if group == "admin" and not is_admin:
            continue
        if group == "user" and is_admin:
            continue
        description = descriptions.get(key)
        if description:
            lines.append(format_jinmups_command(cmds, description))
    return lines


def jinmups_help_string(group="user"):
    from ..telegram_helper.ui_themes import blockquote

    title = "♧ Admin Commands" if group == "admin" else "♧ User Commands"
    lines = _jinmups_help_lines(group)
